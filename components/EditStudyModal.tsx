...
const EditStudyModal: React.FC<Props> = ({ isOpen, onClose, study }) => {
  ...
  const addDose = () => {
    const newDose: DoseData = { dose: '', weightLossPercent: 0, nauseaPercent: 0, vomitingPercent: 0, diarrheaPercent: 0, constipationPercent: 0, saePercent: 0 };
    ...
    try {
      ...
      const cleanedData = {
          ...
          doses: dataToUpdate.doses.map((d: any) => ({
              ...
              saePercent: Number(d.saePercent) || 0,
          }))
      };
        ...
        <form onSubmit={handleSubmit} className="flex-grow contents">
          <div className="p-6 max-h-[70vh] overflow-y-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
              ...
              <div>
                <label htmlFor="edit_formulation" className={formLabelStyle}>剂型</label>
                <select id="edit_formulation" name="formulation" value={formData.formulation} onChange={handleChange} className={formInputStyle}>
               ...
               <div><label className={formLabelStyle}>给药频率</label><input type="text" name="frequency" value={formData.frequency} onChange={handleChange} className={formInputStyle} placeholder="例如: 每周一次" /></div>
                  ...
                  {formData.doses.map((dose, index) => (
                    <div key={index} className="relative p-4 border border-slate-200 rounded-lg bg-slate-50/50">
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-3">
                        ...
                        <div>
                          <label className="block text-xs font-medium text-slate-600 mb-1">SAE (%)</label>
                          <input type="number" name="saePercent" placeholder="严重不良反应" step="0.1" value={dose.saePercent} onChange={e => handleDoseChange(index, e)} className={formInputStyle + " mt-0"} />
                        </div>
                      </div>
